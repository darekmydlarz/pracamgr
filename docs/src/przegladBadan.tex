...
dowiedzieć.
Na końcu skupiono się na opisie serwisu społecznościowego Twitter, który
...
\subsubsection{Pośrednictwo \ang{betweenness} }  
...
jest najmniejsza. Wielkość tę wyraża się wzorem:
...
Wartośc \textit{betweenness} dla wierzchołka B \refimg{image:betweenness}
oblicza się przy pomocy najkrótszych ścieżek między węzłami innymi niż $B$, to
jest: $ABC,$ $ABD$, $ABE$, $CBD$, $CBE$, $DE$.
\mbox{W 5 z 6 z nich} znajduje się węzeł $B$, stąd jego wartość
\textit{betweenness} jest równa $5/6$.
...
Dlatego też mogą być skutecznym celem ataków.
...
Przykładowy graf \refimg{image:closeness} i tabela (tab. \ref{tab:closeness}) z
...
o największej wartości bliskości jest węzeł $B$. Jest to więc
...
Przykładowe wartości wielkości \textit{eigenvector} zaprezentowano na rysunku
\ref{image:eigenvector}\footnote{Obliczone przy pomocy narzędzia Gephi --
...
\textit{Grupa to dwie lub więcej osób, między którymi dłużej niż przez parę
chwil dochodzi do interakcji, które wzajemnie na siebie oddziałują oraz spostrzegają
...
długotrwałych obliczeń. Istnieją jednak prostsze metody, takie jak między
innymi:
...
\begin{itemize}
  ...
  \item zastosowanie 3 rodzajów metryk i balansowanie ich istotnością
  \cite{FindingCommunityStructure},
...
modularności jest największy. Ten proces jest wielokrotnie powtarzany aż do
