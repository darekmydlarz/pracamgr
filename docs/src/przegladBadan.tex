...
wszystkich członków sieci. Dzięki temu możliwe jest rożne podejście do analizy
konkretnych grup i ich zachowań. Istnieje wiele sposobów odkrywania takich grup,
między innymi:

\begin{itemize}
  \item budowanie drzewa odległości (ang. \textit{distance tree}) między 
  użytkownikami na podstawie macierzy relacji między nimi [Combining Social Network Analysis and Web.pdf],
  [TODO Rysunek]
  
  \item wydorębnianie grup na podstawie modularności \cite{FastUnfoldingOfCommunites} -- 
  \textbf{metoda zastosowana w szukaniu grup w niniejszej pracy}
\end{itemize}
